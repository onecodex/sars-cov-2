...
colnames=['region','position','ref allele','alt allele','frequencies','protein sequence variant']

df_variants = pd.read_csv('snpeff.tsv', skiprows=[0], names=colnames, sep='\t| ', engine='python')

for i in df_variants.index:
	ref_reads = int(df_variants.loc[i,'frequencies'].rsplit(",",3)[0]) + int(df_variants.loc[i,'frequencies'].rsplit(",",3)[1])
	alt_reads = int(df_variants.loc[i,'frequencies'].rsplit(",",3)[2]) + int(df_variants.loc[i,'frequencies'].rsplit(",",3)[3])
