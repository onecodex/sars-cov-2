...
df_variants = pd.DataFrame(columns = ['region', 'position', 'ref', 'alt', 'depth', 'alt frequency', 'S/N', 'orf', 'AA mutation'])

# Import the Medaka-generated vcf file. These are our variants
df_variants = pd.read_csv(wd+samplename+'.pass.snpeff.vcf.noheaders', sep='\t', header=None, usecols=[0,1,3,4,7], names=['region','position','ref','alt','info_vcf'])

# Import the bcf file. The info column will give us the allele frequency
df_bcf = pd.read_csv(wd+samplename+'.sorted.bcf.noheaders', sep='\t', header=None, usecols=[0,1,3,4,7], names=['region','position','ref','alt','info_bcf'])
...
# Add in orf info
df_orfs = pd.read_csv('/annot_table.orfs.txt', sep='\t', header=None, usecols=[0,1,2], names=['orf','start','stop'])
for i in df_variants.index:
